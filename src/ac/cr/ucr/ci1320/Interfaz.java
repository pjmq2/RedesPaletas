            ...
            try {
                ...
                if (mensaje.split("\\n").length == 4) {
                    ...
                    if (mensajer != null) {
                        ...
                        if (mensajer.getIpDestino().equals(nodo.getIP())) {
                            casosDePaquetes(finale, lastClientRealIP, true);
                        }
                        else {
                            Solicitante solicitante = new Solicitante(nodo, finale, mensajer.getIpDestino(), mensajer.getAccion()); // Address Port Menssage
                            solicitante.run();
                        }
                    } else {
                    ...
                    if (paquete != null) {
                        ...
                        if (paquete.getMensaje().getIpDestino().equals(nodo.getIP())) {
                            casosDePaquetes(finale, lastClientRealIP, true);
                        }
                        else {
                            Solicitante solicitante = new Solicitante(nodo, finale, paquete.getIpDestinPaquete(), paquete.getMensaje().getAccion()); // Address Port Menssage
                            solicitante.run();
                        }
                    } else {
